...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
                 ...
                 use_checkpoint=False, is_base=True, is_s_pool=True, 
                 pe_dim=128, is_coord=False, is_LSA=False,
                 eps=0., merging_size=2, n_trans=4, 
                 STT_head=16, STT_depth=1, is_rpe=False, is_ape=False
                 ,**kwargs):
        super().__init__()
        ...
        if is_base:
            if not self.is_coord:
               ...
               self.patch_embed = self.to_patch_embedding = nn.Sequential(
                    Rearrange('b c (h p1) (w p2) -> b (h w) (p1 p2 c)', p1 = patch_size, p2 = patch_size),
                    CoordLinear(3*(patch_size**2), embed_dim, exist_cls_token=False)
                )   
